...
# %%
import os
os.chdir('/home/ivan/Dropbox/Research/ELEX/brat')
...
for i in range (2,np.size(DATA.wa)+1):
    indx = np.where(DATA.upper == i)
    ATOT[i] = sum(DATA.aval[indx])

## This array stores branching ratios for every transition in DATA_mat.
## BRAT(i-->j) = AVAL(i-->j) / sum(AVAL(i-->k))
ratio_mat = AVAL_MAT[AVAL_MAT[:, 0] >= DXU, :]
...
#    temp_mat2 = temp_mat2[np.argsort(temp_mat2[:,1])]
    ...
    for j in range(0,np.size(temp_mat1[:,0])):
        for k in range(0, np.size(temp_mat2[:,0])):
            tmp = [[ temp_mat2[k,0], temp_mat2[k,1],temp_mat2[k,2] \
            * temp_mat1[j,2] ]] 
            count = temp_mat2[k,2] * temp_mat1[j,2]
            if count > ZZERO:
                tmp19 = np.append(tmp, tmp19, axis =0)
#            print(tmp19)
